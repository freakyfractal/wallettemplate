...
import wallettemplate.wallets.dogecoin.controls.ClickableDogecoinAddress;

import java.math.BigInteger;
import java.util.Date;

import static wallettemplate.wallets.dogecoin.Dogecoin.kit;
import static wallettemplate.wallets.dogecoin.utils.GuiUtils.checkGuiThread;

/**
 * Gets created auto-magically by FXMLLoader via reflection. The widget fields are set to the GUI wallettemplate.wallets.bitcoin they're named
...
public class DogecoinController {
    ...
    public ClickableDogecoinAddress addressControl;
    ...
    public void onBitcoinSetup() {
        kit.wallet().addEventListener(new BalanceUpdater());
        addressControl.setAddress(kit.wallet().getKeys().get(0).toAddress(Dogecoin.params).toString());
        refreshBalanceLabel();
        //content.setText(kit.wallet().toString());
    }

    public void sendMoneyOut(ActionEvent event) {
        // Hide this UI and show the send money UI. This UI won't be clickable until the user dismisses send_money.
        Dogecoin.instance.overlayUI("send_dogecoin.fxml");
    }
    ...
    public class ProgressBarUpdater extends DownloadListener {
        ...
        protected void doneDownload() {
            super.doneDownload();
            Platform.runLater(DogecoinController.this::readyToGoAnimation);
        }
    }
    ...
    public void refreshBalanceLabel() {
        final BigInteger amount = kit.wallet().getBalance(Wallet.BalanceType.ESTIMATED);
